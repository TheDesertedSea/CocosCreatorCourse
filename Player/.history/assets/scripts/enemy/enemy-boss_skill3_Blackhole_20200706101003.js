...
cc.Class({
    ...
    start () {
        this.time = 0;
        this.initSpeed = this.player.getComponent("Player").speed;

        // this.scheduleOnce(this.explode, 1);
        this.explode();
        // this.schedule
    },
    ...
    update (dt) {
        ...
        if (this.time > this.lifetime) {
            // console.log("destroy");
            this.player.getComponent("Player").speed = this.initSpeed;
            this.node.destroy();
        }
        // this.explode();
        // this.node.angle -= 2;
    ...
    explode() {
        //播放动画
        this.node.getComponent(cc.Animation).play("Blackhole");

        this.player.getComponent("Player").speed = 1;
